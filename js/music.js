...
{
  if(line_num1) {crack_make(line_num1);}
  if(line_num2) {crack_make(line_num2);}
  if(line_num3) {crack_make(line_num3);}
  if(line_num4) {crack_make(line_num4);}
},
...
crack_make:function(line_number)
{
                   ...
                   if(number==1) {line1target[line1lastindex]=num;line1lastindex++;}
                   else if(number==2) {line2target[line2lastindex]=num;line2lastindex++;}
                   else if(number==3) {line3target[line3lastindex]=num;line3lastindex++;}
                   else {line4target[line4lastindex]=num;line4lastindex++;}
                   position[num-1]=0;
...
line2check:function()
 ...
 move:function()
            ...
            dis=position[line1target[i-1]-1];
		    		$('#target'+line1target[i-1]).css({'top':dis-height/2+'px', });
            if(dis>heightjudge)
            {
                  $('#target' + line1target[line1firstindex-1]).remove();
                  line1status=-1;
           ...
           for(var i=line2firstindex;i<=line2lastindex;i++)
          {
            ...
            $('#target'+line2target[i-1]).css({'top':dis-height/2+'px', });
                        if(dis>heightjudge)
            {
                  $('#target' + line2target[line2firstindex-1]).remove();
                  line2status=-1;
          ...
          for(var i=line3firstindex;i<=line3lastindex;i++)
          {
            ...
            $('#target'+line3target[i-1]).css({'top':dis-height/2+'px', });
                        if(dis>heightjudge)
            {
                  $('#target' + line3target[line3firstindex-1]).remove();
                  line3status=-1;
          ...
          for(var i=line4firstindex;i<=line4lastindex;i++)
          {
            ...
            $('#target'+line4target[i-1]).css({'top':dis-height/2+'px', });
             if(dis>heightjudge)
            {
                  $('#target' + line4target[line4firstindex-1]).remove();
                  line4status=-1;
...
/*			var moveNum=Wrapper.randoms();
            ...
            var time=setInterval (function()
            	...
            	if(y>heightjudge){var shortT=setTimeout(function(){$('#target' + idNum).remove();clearInterval(time);shortT=null;},200);clearInterval(time);
            }
time=null;
                 },40);
...
scorecheck:function()
{
if(line1status==-1) {gamestatus=-1;line1status=0;this.colorprocess(1,-1);}
else if(line1status==1) {score+=5;gamestatus=1;line1status=0;this.colorprocess(1,1);scorechangeflag=1;}
else {gamestatus=0;this.colorprocess(1,0)}

if(line2status==-1) {gamestatus=-1;line2status=0;this.colorprocess(2,-1);}
else if(line2status==1) {score+=5;gamestatus=1;line2status=0;this.colorprocess(2,1);scorechangeflag=1;}
else {gamestatus=0;this.colorprocess(2,0);}

if(line3status==-1) {gamestatus=-1;line3status=0;this.colorprocess(3,-1);}
else if(line3status==1) {score+=5;gamestatus=1;line3status=0;this.colorprocess(3,1);scorechangeflag=1;}
else {gamestatus=0;this.colorprocess(3,0);}

if(line4status==-1) {gamestatus=-1;line4status=0;this.colorprocess(4,-1);}
else if(line4status==1) {score+=5;gamestatus=-1;line4status=0;this.colorprocess(4,1);scorechangeflag=1;}
else {gamestatus=0;this.colorprocess(4,0);}
}
,
...
colorprocess:function(line_num,color_status)
{
    ...
    {$('#button'+line_num).css('background-image',"url('./game_picture/perfect.jpg')").css('opacity',0.5);}
    	...
    	{$('#button'+1).css('background-image',"url('./game_picture/button.png')").css('opacity',0.5);line1colorstatus=0;}
        if(line2colorstatus==color_times)
    	{$('#button'+2).css('background-image',"url('./game_picture/button.png')").css('opacity',0.5);line2colorstatus=0;}
        if(line3colorstatus==color_times)
    	{$('#button'+3).css('background-image',"url('./game_picture/button.png')").css('opacity',0.5);line3colorstatus=0;}
        if(line4colorstatus==color_times)
    	{$('#button'+4).css('background-image',"url('./game_picture/button.png')").css('opacity',0.5);line4colorstatus=0;}

    if(color_status!=0)
    {
    	if(line_num==1) {line1colorstatus=0;}
    	else if(line_num==2) {line2colorstatus=0;}
    	else if(line_num==3) {line3colorstatus=0;}
    	else {line4colorstatus=0;}
    }
    else
    {
    	if(line_num==1) {line1colorstatus++;}
    	else if(line_num==2) {line2colorstatus++;}
    	else if(line_num==3) {line3colorstatus++;}
    	else {line4colorstatus++;}
    }
}
...
$('#button1').click(function(){
  if(line1lastindex>=line1firstindex)
	...
	if(distance<=perfectdistance) {line1status=1;}
    else {line1status=-1;}
    $('#target' + line1target[line1firstindex-1]).remove();
 		line1firstindex++;}
...
$('#button2').click(function(){
  if(line2lastindex>=line2firstindex)
	...
	if(distance<=perfectdistance) {line2status=1;}
    else {line2status=-1;}
    $('#target' + line2target[line2firstindex-1]).remove();
 		line2firstindex++;}
...
$('#button3').click(function(){if(line3lastindex>=line3firstindex)
	{
    distance=Math.abs(position[line3target[line3firstindex-1]-1]-(LineLength+height/5));
	if(distance<=perfectdistance) {line3status=1;}
    else {line3status=-1;}
    $('#target' + line3target[line3firstindex-1]).remove();
 		line3firstindex++;}
...
$('#button4').click(function(){if(line4lastindex>=line4firstindex)
	{
    distance=Math.abs(position[line4target[line4firstindex-1]-1]-LineLength);
	if(distance<=perfectdistance) {line4status=1;}
    else {line4status=-1;}
    $('#target' + line4target[line4firstindex-1]).remove();
 		line4firstindex++;}
...
pausecheck:function()
{
  ...
  {
    if(is_pause){is_pause=0;$('#pause_button').css('background-image',"url('./game_picture/pause.png')");play_flag=1;}
      else {is_pause=1;$('#pause_button').css('background-image',"url('./game_picture/play.png')");pause_flag=1;}
      pause_change_flag=0;
  }
},
...
var timer=setInterval(function(){
  if(!is_pause)
   {
    if(play_flag){play_flag=0;audio.play();}
  //  if(times%2==0)
  //  	{if(flag)
  //  	{Wrapper.randoms();}}
  ...
  //  if(looptimes%circle_looptimes==0){flag=1}
    if((looptimes*2)%circle_looptimes==0){times+=1;timeschangeflag=1;
  ...
  // if(times==188){clearInterval(timer);}
   ...
   {
    if(pause_flag){pause_flag=0;audio.pause();}
    Wrapper.pausecheck();
   }
